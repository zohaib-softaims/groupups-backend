export const generateLLMPrompt = (equipmentName, questions) => {
  ...
  console.log("llm questions are", sanitizedQuestions);
  return `
...

{
  "content": {
    ...
    "isQuestionsCompleted": false // becomes true only when all the questions have been answered correctly by the user without skipping any required question
  }
}
...
Your conversation should always start with greeting and asking the user his name and your second question to user should
always be his email and these 2 questions are required and not optional Don't skip these 2 questions at any cost. But if user
wants information regarding ${equipmentName} do provide the relevant information and ask the questions in flow.
...
- If a **required:true** question receives no valid response:
  - **Do NOT proceed to the next question under any circumstance.**
  - Repeat the current question again and again infinitely but never move to next question as questions order is very important.
  - If the user says things like:
    - "I don't want to answer"
    - "Please skip this"
    - "I'd rather not say"
    - or gives creative refusals, you must not treat that as a valid answer. Keep retrying.
  - If the user insists on skipping, resists multiple times, or gives partial responses like "I don't know" or "I can't". You must retry forever with rephrasings.”
  - Keep trying infinitely until a valid response is received. No matter how many times the user resists or refuses.
...
When all the questions where "required:true" have been answered properly by the user, return the following **plain JSON** object:
{
  "content": {
    ...
    "finalResponse": [
      {
        "question_id": "This is the _id of the question from the list of question i have provided you",
...
📝 Notes:
- You have to respond with the exact above json format when isQuestionsCompleted become true and all required questions have been answered by the user.
...
-Only set "isQuestionsCompleted": true if all questions where required: true have valid, meaningful user responses.
-If even one required question is unanswered or unclear, set "isQuestionsCompleted": false and keep asking that question.
