export const generateLLMPrompt = (
  ...
  const generalContextString =
    generalContext?.map((snippet) => snippet.snippet_text).join("\n") || "";
  const questionContextString =
    questionContext?.map((snippet) => snippet).join("\n") || "";
...
- **image**: (optional) A URL or base64 string of an image to show with the question, if provided.
...

{
  "content": {
    ...
    "image": "image_url_or_base64_if_present" // (optional) include if the question has an image
  }
}
...
When all questions have been answered with valid responses, reply with this exact message:
{
  "content": {
    "responseText": "${
      endingMessage
        ? endingMessage
        : "Based on the information you've shared, we've identified suitable options that you should demo."
    }",
...
If you find no questions to ask, or if the list of questions is empty, reply with this exact message:
{
  "content": {
    "responseText": "Currently, I can't help you with this. Please come back later!!!",
    "question_type": "open_ended",
    "isQuestionsCompleted": "true"
  }
}
